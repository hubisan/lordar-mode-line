...
- lordar-mode-line-segments-git-branch (&optional format-string) :: Return the vc branch formatted to display in the mode line. Use format-string to change the output. Notice: If switching the branch it is not guaranteed to be updated immediately. 
  
- lordar-mode-line-segments-git-state (&optional format-string) :: Return an indicator representing the vc status of the current buffer. Use format-string to change the output. The indicators defined in lordar-mode-line-git-state-symbols are used.
...
- lordar-mode-line-git-state-symbols :: Symbols for buffer status in the mode line. Each entry is a cons cell with a keyword and a corresponding string. Default:
  #+BEGIN_SRC emacs-lisp
    (setq lordar-mode-line-git-state-symbols
          ((up-to-date) (edited . "*")
